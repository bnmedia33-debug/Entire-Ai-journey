...
export default function Chat({ user }: ChatProps) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userMsgPath = "chat_history";
      try {
      ...
      let context = "";
      try {
        const kbQuery = query(collection(db, kbPath), where("subject", "==", selectedSubject));
        ...
        handleFirestoreError(err, OperationType.GET, kbPath);
      }
      ...
      // 4. Save AI response to Firestore after streaming completes
      try {
        ...
        handleFirestoreError(err, OperationType.CREATE, "chat_history");
      }
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-8 scroll-smooth">
          <div className="max-w-4xl mx-auto space-y-8">
            ...
            {streamingResponse && (
              ...
              >
                <div className="max-w-[85%] flex gap-5 flex-row">
                  <div className="w-10 h-10 rounded-2xl flex-shrink-0 flex items-center justify-center bg-indigo-600 text-white shadow-indigo-100">
                    <Bot className="w-5 h-5" />
                  </div>
                  <div className="p-6 rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] bg-white border border-zinc-100 text-zinc-800 rounded-tl-none">
